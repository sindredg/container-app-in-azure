...
Related: [immutable image tags, `linux/amd64` builds, and read-only runtime access](../decisions.md).
...
Proves the push permission was a scoped role assignment on the registry, not enabled admin credentials.

![Docker client authenticated to the private registry](../images/phase-07-acr-login.png)

Proves `az acr login` succeeded without enabling registry administrator credentials.
...
Proves the build ran for `linux/amd64` and pushed straight to the registry.

![The 0.1.0 manifest contains a linux amd64 image](../images/phase-07-web-010-manifest-amd64.png)

Proves the published manifest reports `linux/amd64`, so the image matches the Azure runtime rather than the build machine. Tag `0.1.1` was verified the same way.
...
Proves the plan added only the Container App and did not replace the environment, registry, or identity.

![Terraform creates the web Container App and returns its URL](../images/phase-07-apply-complete.png)

Proves the apply added exactly 1 resource and returned the public HTTPS hostname as an output.

![The initial Container Scale Lab page runs in Azure](../images/phase-07-first-deployment.png)

Proves public ingress served the `0.1.0` page over the Azure-managed hostname.

![Azure reports the initial web revision as succeeded](../images/phase-07-revision-succeeded.png)

Proves Azure accepted and started the application configuration.
...
Proves the rebranded page was served locally on port `8081` before it was published.

![Docker reports the local branded container as healthy](../images/phase-07-branded-local-healthy.png)

Proves the `0.1.1` container passed its own health check locally.

![The Terraform web image variable selects immutable tag 0.1.1](../images/phase-07-image-tag-variable-011.png)

Proves the rollout is driven by one declared variable rather than an edited resource block.

![Docker builds and pushes branded image 0.1.1](../images/phase-07-web-011-build-push.png)

Proves `0.1.1` was published as a separate tag, leaving `0.1.0` intact as a rollback target.

## Stale state lease

![Terraform cannot acquire the stale Azure Blob state lease](../images/phase-07-stale-lock-error.png)

Proves the failure was a stale blob lease with empty `terraformlockid` metadata, which is why `force-unlock` could not be used.
...
Proves the lease was broken on `platform/dev.tfstate` and returned `0`, meaning no break period remained.

![Terraform plans one update for web image 0.1.1](../images/phase-07-web-011-plan-update.png)

Proves the tag change was an in-place update, not a resource replacement.

![Terraform applies the web image update](../images/phase-07-web-011-apply-complete.png)

Proves the apply changed one resource and destroyed nothing, so only the template moved.

![Cloud operations Lab runs on Azure Container Apps](../images/phase-07-branded-public.png)

Proves the branded content was live on the same public hostname after the revision rollout.

![The branded Azure deployment passes its health request](../images/phase-07-branded-health-200.png)

Proves `/health` returned `200` over HTTP/2 through Azure ingress after the update.

![The active branded revision is healthy and receives all traffic](../images/phase-07-revision-healthy.png)

Proves the revision was active, healthy, provisioned, and holding 100 percent of traffic on one replica.

## Post-deployment validation

![Ten identifiable HTTPS requests return HTTP 200](../images/phase-07-validation-http-traffic-200.png)

Proves the endpoint handled a tagged request sequence, giving each request a marker to find in the logs.
...
Proves the tagged requests can be traced to a named revision and container, with the ingress proxy address and the originating client address both in the log line.

![Container Apps system logs show scale, scheduling, image pull, and startup](../images/phase-07-validation-scale-from-zero.png)

Proves scale-to-zero is active and a request triggered a new replica: scaled 0 to 1, image `web:0.1.1` pulled in 1.549s, container created and started.

![Nonexistent and sensitive-looking paths return HTTP 200](../images/phase-07-validation-negative-path-200.png)

Proves the routing defect: `/admin`, `/login`, `/.env`, `/.git/config`, and `/server-status` all returned `200` instead of `404`.

![Log Analytics captures all five negative-path requests](../images/phase-07-validation-negative-path-logs.png)

Proves the probes were observable, each returning `200` with a 1621-byte body, which is the homepage size and confirms the fallback served the index page rather than exposing real files.

The cause was `try_files $uri $uri/ /index.html`. This finding drove phase 8.
