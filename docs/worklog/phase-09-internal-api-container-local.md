...
The image builds from `python:3.13-slim` and adds a dedicated `appuser` with uid `10001` rather than running as root.

![The API reports its identity through /status](../images/phase-09-api-status-direct.png)

`/status` returns structured JSON. Outside Azure the revision and replica fields fall back to `local` rather than failing.
...
`nginx.conf` became `default.conf.template` through `git mv`, so file history follows the rename.

![The web Dockerfile restricts envsubst to API_ names](../images/phase-09-web-dockerfile-envsubst.png)

`NGINX_ENVSUBST_FILTER=^API_` is set. Without it, envsubst substitutes every environment variable and destroys `$uri`, `$status`, and the whole privacy log format from phase 8.
...
The browser calls the API same-origin. `X-Forwarded-For` is emptied so the API never sees a client address, and the truncated `/24` network goes through as `X-Client-Network`.
...
The topology: the web container maps `8080` to the host, and the API container exposes `8080` inside the network with no host binding, so it is unreachable except through the proxy.

![The API answers through the web origin](../images/phase-09-api-status-through-proxy.png)

`/api/status` on the web port returns the API's JSON, so the proxy path works end to end.

![Security headers survive on a proxied response](../images/phase-09-proxy-headers-preserved.png)

All four phase 8 headers and the bare `nginx` server header are present on a successful proxied response, so adding a `location` did not drop the inherited `add_header` set.

![A missing API path returns the API's own 404](../images/phase-09-proxy-404-passthrough.png)

An unmatched API route returns the FastAPI `404` as `application/json` rather than the static site's HTML 404, so the two routing domains stay separate.

![HEAD is rejected because the route registers GET only](../images/phase-09-head-405-method-not-allowed.png)

`curl -I` returns `405 Method Not Allowed` with `allow: GET`. FastAPI's `@app.get` registers GET alone, unlike plain Starlette, so header checks against this API need a GET that discards the body.

![Web container logs across the proxy tests](../images/phase-09-compose-logs-web.png)

The phase 8 privacy format still applies to proxied requests, logging `unknown` where no forwarded address exists, and records the full sequence of `/api/` requests including the `405`.
