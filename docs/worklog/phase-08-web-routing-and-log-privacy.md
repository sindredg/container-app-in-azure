...
Full technical detail in [phase 8](../phases/phase-08-web-routing-and-log-privacy.md).

## Before

![Deployed 0.1.1 advertises its Nginx version and sets no protective headers](../images/phase-08-version-disclosed-before.png)

Proves the state this phase corrects: the live site returned `server: nginx/1.29.8` with none of the four headers.
...
Proves `nginx -t` inside the running `0.1.2` container passed, so the added directives sit in a legal context.

![The 0.1.2 container runs and reports healthy](../images/phase-08-local-container-healthy.png)

Proves the new image ran with a passing health check before publication.

![Local response headers show no version and four protective headers](../images/phase-08-local-root-200-headers.png)

Proves `server` is now bare `nginx`, and all four headers are present on a successful response.

![A missing path returns a real 404 with the headers still applied](../images/phase-08-local-missing-path-404.png)

Proves `/admin` returns a genuine Nginx `404` body rather than the homepage, and that the headers survive on an error response, which is what `always` buys.

![Local access log shows truncated and unknown client addresses](../images/phase-08-local-log-truncation.png)

Proves a request from `203.0.113.45` logs as `203.0.113.0`, that the result is identical when the proxy appends its own address, and that a request with no header logs `unknown`.
...
Proves the page now states that request logs are truncated to network level and retained for 30 days.

## Release

![Image 0.1.2 tagged and pushed to the registry](../images/phase-08-image-push-012.png)

Proves the release was published as its own digest.

![The registry holds three immutable release tags](../images/phase-08-acr-tags-three-releases.png)

Proves `0.1.0`, `0.1.1`, and `0.1.2` coexist, so earlier releases remain available as rollback targets.

![The Terraform image variable selects tag 0.1.2](../images/phase-08-image-tag-variable-012.png)

Proves the rollout is driven by the declared variable moving to the new tag.

![Configuration formatted and validated before planning](../images/phase-08-validate-and-plan.png)

Proves `fmt -check` and `validate` both passed before the plan was generated.

![Terraform plans a single in-place change](../images/phase-08-plan-one-change.png)

Proves the release was 0 to add, 1 to change, 0 to destroy.

![Terraform applies the 0.1.2 release](../images/phase-08-apply-complete-012.png)

Proves the apply changed one resource and returned the unchanged public hostname.

## Azure verification

![The deployed site returns no version and four protective headers](../images/phase-08-azure-root-200-headers.png)

Proves the hardening reached production over HTTP/2.

![Log Analytics shows truncated addresses and a 404 for a missing path](../images/phase-08-azure-log-truncation.png)

Proves truncation is active in Azure, and captures an unplanned confirmation of the routing change: a browser request for `/favicon.ico` returned `404 548`, where the previous configuration would have returned `200` with the full homepage.
