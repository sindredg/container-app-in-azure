...
The API is a second `azurerm_container_app` in the same environment and resource group, in single revision mode, following the naming pattern of the web app.
...
The API follows the same immutable tag discipline as the web app, starting at `0.1.0`.
...
`API_UPSTREAM` is derived from the API's own ingress FQDN in Terraform, so the address is never hardcoded and updates automatically if the app is recreated.

![Both apps carry a component tag](../images/phase-10-component-tags.png)

The shared tag set is extended per app, which keeps the two services separable in cost and inventory views.

![Terraform plans the API and the web update together](../images/phase-10-plan-api-create.png)

The change was 1 to add and 1 to change, adding the API and updating the web app, with the internal FQDN only knowable after apply.

## Verification

![The internal FQDN is not reachable from outside the environment](../images/phase-10-internal-fqdn-not-public.png)

A request to the API's own hostname from a workstation returns `404` from the Azure front end rather than the API, so internal ingress is doing its job.

![The proxied path returns nginx's own 404](../images/phase-10-api-status-404-stale-web.png)

The deployed web image did not contain the `/api/` block. The environment variable had applied, but `web_image_tag` was never moved off `0.1.2`, so the running image predated the proxy. Nginx returning its own `404` rather than a `502` is what identified this: a missing location, not a failing upstream.

![The web image tag moves to 0.2.0](../images/phase-10-web-image-tag-020.png)

The fix was a tag bump to the image that actually contains the proxy configuration.

![Terraform applies the web release](../images/phase-10-apply-complete.png)

The correction was a single in-place change with nothing added or destroyed.

![The API answers through the public site with real Azure values](../images/phase-10-api-status-through-public-site.png)

The full path works: a public HTTPS request to `/api/status` reaches the internal API, which reports its own revision and replica names. Those values could only come from inside the Container Apps environment.
...
- `/api/openapi.json` is publicly readable through the proxy.
